...
**Every number in this section is downstream of the developer's hardware.** The defaults (45 tracks, 9 themes, 3 parallel requests) were tuned against **qwen/qwen3.6-27b on a Mac Studio M3 Ultra**, where memory is not the binding constraint. On a Mac with less unified memory the user runs a smaller model with a smaller context, and the same defaults can exceed it — which surfaces as `.contextExceeded` taking out most of a batch, or as themes returning empty. Nothing in the code detects this, and it shouldn't be silently compensated for either: the settings are the remedy, and README's "About your Mac and your model" documents the order to reach for them (parallel requests first, since each in-flight request reserves its own prompt + `max_tokens` of KV cache). **When changing any default here, state what it assumes about the machine** — a value that is safe on 512 GB of unified memory is not self-evidently safe on 16 GB.
